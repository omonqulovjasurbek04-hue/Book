  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        console.error("Fikrlarni olishda xatolik:", error)
        setReviews([])
      } finally {
        setLoading(false)
      }
            ...
            reviews.map((rev, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-6 relative z-10">
                  ...
                  <div>
                    <h4 className="font-semibold text-[#2b2b2b] text-[13px] md:text-[14px]">
                      {rev.name || 'Foydalanuvchi'}
                    </h4>
                    <p className="text-[#8964C0] text-[12px] md:text-[13px] font-medium mt-0.5">
                      {rev.course_name || 'Kurs o‘quvchisi'}
                    </p>
                  </div>
                </div>

                <p className="text-[#333] text-[13px] md:text-[14px] leading-[1.6] font-medium relative z-10 w-[95%]">
                  {rev.message_uz || rev.message || "Sharh matni yo'q"}
                </p>
              </div>
            ))
          )}
